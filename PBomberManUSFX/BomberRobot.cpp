#include "BomberRobot.h"


BomberRobot::BomberRobot(Texture* _textura, Tile* _tileActual) :GamePawn(_textura, _tileActual)
{
	...
	if (tileActual != nullptr) {
		tileActual->setBomberRobot(this);
...
void BomberRobot::update2()
{
	direccionSiguiente = MOVE_DIRECTION_NONE;

	if (keyboardInput2->IsKeyOn(botonAbajo2)) {
		direccionSiguiente = MOVE_DIRECTION_ABAJO;
	}
	else if (keyboardInput2->IsKeyOn(botonArriba2)) {
		direccionSiguiente = MOVE_DIRECTION_ARRIBA;
	}
	else if (keyboardInput2->IsKeyOn(botonDerecha2)) {
		direccionSiguiente = MOVE_DIRECTION_DERECHA;
	}
	else if (keyboardInput2->IsKeyOn(botonIzquierda2)) {
...
void BomberRobot::render()
{
	GamePawn::render();
}
void BomberRobot::setTileActual(Tile* _tileNuevo )
{
	if (tileActual != nullptr) {
